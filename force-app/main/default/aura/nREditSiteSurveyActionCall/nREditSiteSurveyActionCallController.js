({
    doInit : function(component, event, helper) {
        var action = component.get("c.getSiteSurveyDetails");
        action.setParams({"siteSurveyId": component.get("v.recordId")});
        
        action.setCallback(this, function(response){
            var state = response.getState();
            if (state === "SUCCESS"){
                var siteSurveyIns = response.getReturnValue();
                
                if(siteSurveyIns.NR_Status__c === 'New'){
                    ...
                    toastEvent.setParams({
                        title: "WARN!",
                        type: 'error',
                        message: "Status [" + siteSurveyIns.NR_Status__c + "] is not applicable to edit."
                    });
                    toastEvent.fire();
                }
